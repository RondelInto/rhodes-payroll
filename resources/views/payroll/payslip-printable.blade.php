<div class="p-8 font-sans">
    ...
    <table class="w-full mt-6 border-collapse">
        ...
        <tr><td>Overtime Pay</td><td class="text-right">₱{{ number_format($transaction->overtime_pay,2) }}</td><td class="pl-4">PhilHealth</td><td class="text-right">₱{{ number_format($transaction->philhealth_contribution,2) }}</td></tr>
        <tr><td>Holiday Pay</td><td class="text-right">₱{{ number_format($transaction->holiday_pay,2) }}</td><td class="pl-4">Pag-IBIG</td><td class="text-right">₱{{ number_format($transaction->pagibig_contribution,2) }}</td></tr>
        <tr><td>Allowances</td><td class="text-right">₱{{ number_format(array_sum((array)$transaction->allowances),2) }}</td><td class="pl-4">Withholding Tax</td><td class="text-right">₱{{ number_format($transaction->withholding_tax,2) }}</td></tr>
        <tr class="border-t font-bold"><td>Gross Pay</td><td class="text-right">₱{{ number_format($transaction->gross_pay,2) }}</td><td class="pl-4">Total Deductions</td><td class="text-right">₱{{ number_format($transaction->total_deductions,2) }}</td></tr>
